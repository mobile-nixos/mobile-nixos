...
`outputs.default` target provides a file `extlinux-boot.img` which is
an image of such a fileystem. A full install of Mobile NixOS comprises installing lk2nd (if not installed already), then  writing `extlinux-boot.img` and
...
$ dd status=progress if=extlinux-boot.img | adb shell -e none dd of=/dev/block/mmcblk1p1 bs=1048576
