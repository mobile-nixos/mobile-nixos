...
=== Firmware for Wi-Fi and modem

This particular phone keeps the firmware files on partitions named
`modem` and `fsg` (and possibly others we don't know about yet).
...
alternate recovery like TWRP, mount each partition (identified using
...
$ cp -r /mnt ./modem
$ sudo umount /mnt
$ sudo mount -o ro /dev/disk/by-partlabel/fsg /mnt
$ cp -r /mnt ./fsg
$ sudo umount /mnt
```

The copy of the firmware files will be in the `modem` and `fsg`
subdirectories of the current working directory. Please refer to
devices/motorola-potter/firmware/default.nix for instructions
on how to use it.
